...
pnpm install           # Install dependencies (alternative)
...
```bash
# Validation
shellcheck script.sh                    # Check for errors
shfmt -i 2 -w script.sh                 # Format
shellharden script.sh                   # Harden for security
...
- **Dependency audit**: `uv audit` (Python), `npm audit` (JavaScript)
- **No generic error handling**: Catch specific exceptions; include context
- **No code generation without matching tests**

### CI/CD Security
- **Action pinning**: Use version tags (e.g., `actions/checkout@v4`), not branches
...
|----------|----------|------------|--------|-----------|------------|------|
| **JavaScript/TypeScript** | `bun` (runner) | `pnpm` | `biome` | `biome` | `typescript --strict` | `vitest` |
...
- **Manual**: `uv lock` (Python), `pnpm update` (Node)
- **Audit**: Always run `uv audit` / `npm audit` after updates
...
**Organization**: Ven0m0
**See Also**: `AGENTS.md`, `CONTRIBUTING.md`, local `CLAUDE.md`
